...
age_table <- age_table %>%
  mutate(
    sd = sqrt(age_var$child_age_months),
    age_print = sprintf("%.1f (%.1f)", child_age_months, sd)
  ) %>%
...
## adjusted
fit_was_all <- svyglm(wasting_bin ~ postcovid + child_age_months + caste + religion,
                      design = des_child_bio, family = quasibinomial())
fit_an_all  <- svyglm(anemia_bin  ~ postcovid + child_age_months + caste + religion,
                      design = des_child_bio, family = quasibinomial())

OR_was_alla <- or_post(fit_was_all)
...
## adjusted
fit_was_by_q <- lapply(1:5, function(q) {
  svyglm(wasting_bin ~ postcovid + child_age_months + caste + religion,
         design = subset(des_child_bio, wi_combined_urbanrural == q),
         family = quasibinomial())
})
OR_was_by_qa <- do.call(rbind, lapply(fit_was_by_q, or_post))
rownames(OR_was_by_qa) <- paste0("Q", 1:5)

fit_an_by_q <- lapply(1:5, function(q) {
  svyglm(anemia_bin ~ postcovid + child_age_months + caste + religion,
         design = subset(des_child_bio, wi_combined_urbanrural == q),
         family = quasibinomial())
})
...
################################################################################
# 8b) ADJUSTED (marginally standardized) PREVALENCE — children
################################################################################

adj_prev_sub <- function(formula, dq, wtvar = "child_weight") {
...
adj_was_overall <- adj_prev_sub(wasting_bin ~ postcovid + child_age_months + caste + religion, des_child_bio)
adj_an_overall  <- adj_prev_sub(anemia_bin  ~ postcovid + child_age_months + caste + religion, des_child_bio)

adj_was_by_q <- t(sapply(1:5, function(q)
  adj_prev_sub(wasting_bin ~ postcovid + child_age_months + caste + religion,
               subset(des_child_bio, wi_combined_urbanrural == q))))
rownames(adj_was_by_q) <- paste0("Q", 1:5)

adj_an_by_q <- t(sapply(1:5, function(q)
  adj_prev_sub(anemia_bin ~ postcovid + child_age_months + caste + religion,
               subset(des_child_bio, wi_combined_urbanrural == q))))
rownames(adj_an_by_q) <- paste0("Q", 1:5)

################################################################################
# 9) Weighted concentration indices + curve data
################################################################################
...
cat("\n=== ADJUSTED vs CRUDE PREVALENCE (Wasting, children) ===\n")
cat("Overall  crude  pre/post:", round(as.numeric(prev_was_pre)[2],1),
    round(as.numeric(prev_was_post)[2],1), "\n")
cat("Overall  adj    pre/post:", round(adj_was_overall[["pre"]],1),
    round(adj_was_overall[["post"]],1), "\n\n")
cat("By quintile — ADJUSTED (pre, post):\n"); print(round(adj_was_by_q,1))
cat("\nBy quintile — CRUDE pre:\n");  print(round(was_by_q_pre[2,],1))
cat("By quintile — CRUDE post:\n");   print(round(was_by_q_post[2,],1))

cat("\n=== ADJUSTED vs CRUDE PREVALENCE (Anemia, children) ===\n")
...
wasting_children <- data.frame(
  ...
  aOR = c(0.83, 0.81, 0.77, 0.74, 0.66, 0.76),
  Lower_CI = c(0.77, 0.76, 0.71, 0.68, 0.57, 0.73),
  Upper_CI = c(0.88, 0.88, 0.84, 0.81, 0.76, 0.79)
)
...
ggplot(wasting_children, aes(y = Group, x = aOR)) +
  geom_point(size = 4) +
  geom_errorbar(
  ...
  coord_cartesian(xlim = c(0.50, 1.48), clip = "off") +
...
anemia_children <- data.frame(
  Group = c("Q1", "Q2", "Q3", "Q4", "Q5", "Overall"),
  aOR = c(0.73, 0.75, 0.73, 0.73, 0.89, 0.76),
  Lower_CI = c(0.68, 0.70, 0.68, 0.68, 0.83, 0.73),
  Upper_CI = c(0.78, 0.81, 0.79, 0.78, 0.96, 0.78)
)
...
ggplot(anemia_children, aes(y = Group, x = aOR)) +
  geom_point(size = 4) +
  geom_errorbar(
    aes(xmin = Lower_CI, xmax = Upper_CI),
    width = 0.2,
