...
age_table <- age_table %>%
  mutate(
    sd = sqrt(age_var$man_age),
    age_print = sprintf("%.1f (%.1f)", man_age, sd)
  ) %>%
...
# aOR + 95% CI for postcovid effect from svyglm
...
## adjusted
fit_under_all <- svyglm(undernourished_bin ~ postcovid + man_age + caste + religion, 
                        design = des_men_bio, family = quasibinomial())
fit_an_all    <- svyglm(anemia_bin ~ postcovid + man_age + caste + religion,
                        design = des_men_bio, family = quasibinomial())
...
## adjusted ORs
fit_under_by_q <- lapply(1:5, function(q) {
  svyglm(undernourished_bin ~ postcovid + man_age + caste + religion,
         design = subset(des_men_bio, wi_combined_urbanrural == q),
         family = quasibinomial())
})
...
fit_an_by_q <- lapply(1:5, function(q) {
  svyglm(anemia_bin ~ postcovid + man_age + caste + religion,
         design = subset(des_men_bio, wi_combined_urbanrural == q),
         family = quasibinomial())
})
...
################################################################################
# 8b) ADJUSTED (marginally standardized) PREVALENCE — men
#     Same model as the aORs: postcovid + man_age + caste + religion.
################################################################################

# Adjusted prevalence (%) pre and post, via marginal standardization,
# for one design (overall or a quintile subset).

adj_prev_sub <- function(formula, dq) {
  fq <- svyglm(formula, design = dq, family = quasibinomial())
  mf <- model.frame(fq)
  
  w_named <- dq$variables$man_hh_state_weight
  names(w_named) <- rownames(dq$variables)
  ww <- as.numeric(w_named[rownames(mf)])        
  
  d_pre  <- mf; d_pre$postcovid  <- factor("Pre-lockdown",  levels = levels(mf$postcovid))
  d_post <- mf; d_post$postcovid <- factor("Post-lockdown", levels = levels(mf$postcovid))
  
  p_pre  <- as.numeric(predict(fq, d_pre,  type = "response"))  # strip svystat -> numeric
...
# --- Overall
adj_under_overall <- adj_prev_sub(
  undernourished_bin ~ postcovid + man_age + caste + religion, des_men_bio)
adj_an_overall <- adj_prev_sub(
  anemia_bin ~ postcovid + man_age + caste + religion, des_men_bio)

# --- By wealth quintile
adj_under_by_q <- t(sapply(1:5, function(q)
  adj_prev_sub(undernourished_bin ~ postcovid + man_age + caste + religion,
               subset(des_men_bio, wi_combined_urbanrural == q))))
rownames(adj_under_by_q) <- paste0("Q", 1:5)

adj_an_by_q <- t(sapply(1:5, function(q)
  adj_prev_sub(anemia_bin ~ postcovid + man_age + caste + religion,
               subset(des_men_bio, wi_combined_urbanrural == q))))
rownames(adj_an_by_q) <- paste0("Q", 1:5)

################################################################################
# 9) Weighted concentration indices + curve data (FIXED)
################################################################################
...
cat("\n=== ADJUSTED vs CRUDE PREVALENCE (Underweight, men) ===\n")
cat("Overall  crude  pre/post:", round(as.numeric(prev_under_pre)[2], 1),
    round(as.numeric(prev_under_post)[2], 1), "\n")
cat("Overall  adj    pre/post:", round(adj_under_overall[["pre"]], 1),
    round(adj_under_overall[["post"]], 1), "\n\n")
cat("By quintile — ADJUSTED (pre, post):\n"); print(round(adj_under_by_q, 1))
cat("\nBy quintile — CRUDE pre:\n");  print(round(under_by_q_pre[2, ], 1))
cat("By quintile — CRUDE post:\n");   print(round(under_by_q_post[2, ], 1))

cat("\n=== ADJUSTED vs CRUDE PREVALENCE (Anemia, men) ===\n")
cat("Overall  crude  pre/post:", round(as.numeric(prev_an_pre)[2], 1),
    round(as.numeric(prev_an_post)[2], 1), "\n")
cat("Overall  adj    pre/post:", round(adj_an_overall[["pre"]], 1),
    round(adj_an_overall[["post"]], 1), "\n\n")
cat("By quintile — ADJUSTED (pre, post):\n"); print(round(adj_an_by_q, 1))
cat("\nBy quintile — CRUDE pre:\n");  print(round(an_by_q_pre[2, ], 1))
cat("By quintile — CRUDE post:\n");   print(round(an_by_q_post[2, ], 1))
...
undernutrition_men <- data.frame(
  ...
  aOR = c(1.06, 1.25, 1.04, 0.88, 0.83, 1.02),
  Lower_CI = c(0.95, 1.12, 0.93, 0.77, 0.72, 0.96),
  Upper_CI = c(1.18, 1.40, 1.17, 1.00, 0.96, 1.08)
)

undernutrition_men <- undernutrition_men %>%
  mutate(
    RR_label = sprintf("%.2f (%.2f–%.2f)", aOR, Lower_CI, Upper_CI),
    Label_Pos = pmin(aOR + 0.04, 1.34),  # To keep ratios next to respective plots
    Characteristic = factor(Group, levels = rev(Group))  # Preserve order
  )

ggplot(undernutrition_men, aes(y = Group, x = aOR)) +
  geom_point(size = 4) +
  geom_errorbar(
...
anemia_men <- data.frame(
  Group = c("Q1", "Q2", "Q3", "Q4", "Q5", "Overall"),
  aOR = c(0.77, 0.80, 0.77, 0.77, 0.78, 0.78),
  Lower_CI = c(0.70, 0.72, 0.69, 0.69, 0.70, 0.74),
  Upper_CI = c(0.85, 0.89, 0.85, 0.86, 0.88, 0.82)
)
...
ggplot(anemia_men, aes(y = Group, x = aOR)) +
  geom_point(size = 4) +
  geom_errorbar(
    aes(xmin = Lower_CI, xmax = Upper_CI),
    width = 0.2,
