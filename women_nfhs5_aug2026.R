...
################################################################################
# 8b) ADJUSTED (marginally standardized) PREVALENCE — women
################################################################################

adj_prev_sub <- function(formula, dq, wtvar = "woman_weight") {
...
adj_under_overall <- adj_prev_sub(undernourished_bin ~ postcovid + woman_age + caste + religion, des_women_bio)
adj_an_overall    <- adj_prev_sub(anemia_bin        ~ postcovid + woman_age + caste + religion, des_women_bio)

adj_under_by_q <- t(sapply(1:5, function(q)
  adj_prev_sub(undernourished_bin ~ postcovid + woman_age + caste + religion,
               subset(des_women_bio, wi_combined_urbanrural == q))))
rownames(adj_under_by_q) <- paste0("Q", 1:5)

adj_an_by_q <- t(sapply(1:5, function(q)
  adj_prev_sub(anemia_bin ~ postcovid + woman_age + caste + religion,
               subset(des_women_bio, wi_combined_urbanrural == q))))
...
cat("\n=== ADJUSTED vs CRUDE PREVALENCE (Underweight, women) ===\n")
...
cat("\n=== ADJUSTED vs CRUDE PREVALENCE (Anemia, women) ===\n")
